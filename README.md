...
```
javascript:(function(){window.bookmarkver='1.4';var isReddit=location.hostname.split('.').slice(-2).join('.')==='reddit.com';var isOverview=/\/overview\b/i.test(location.href);if(isReddit&&isOverview){var url='https://raw.githubusercontent.com/rebicul/PowerDeleteSuite/master/powerdeletesuite.js?'+Date.now();fetch(url).then(function(response){if(!response.ok)throw new Error('HTTP '+response.status);return response.text();}).then(function(data){var script=document.createElement('script');script.id='pd-script';script.textContent=data;document.head.appendChild(script);}).catch(function(error){alert('Error retrieving PowerDeleteSuite: '+error.message);});}else if(confirm('Power Delete Suite must run from your Reddit overview. Go there now?')){location='https://old.reddit.com/user/me/overview';}})();
```
